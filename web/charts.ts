...
export const metrics: { key:MetricKey; label:string; unit:string; color:string; domain?:[number,number] }[] = [
  ...
  { key:'treasury',label:'Treasury',unit:'F$',color:'#9e793d' },
...
export function chartSeries(world:World, key:MetricKey) {
  return [createWorld().state,...world.events.filter(event=>!event.cabinetAssignment&&(!event.strategic||event.strategic.command.kind==='overnight'||event.strategic.reports.length)).map(event=>event.after)].map(state=>({day:state.day,value:state[key]}));
}
...
function karmeniaRelations(world:World){
  let score=0;
  return world.events.filter(event=>event.security).map(event=>{const reply=event.security!.reply;const change=reply==='limited_defense'?12:reply==='humanitarian_corridor'?5:reply==='international_monitor'?3:reply==='reject_attack'?-8:0;score=Math.max(-100,Math.min(100,score+change));return {day:event.after.day,position:reply==='limited_defense'?'Limited defensive support':reply==='humanitarian_corridor'?'Humanitarian corridor':reply==='international_monitor'?'International monitors':reply==='reject_attack'?'Rejected attack request':reply==='ask_evidence'?'Requested evidence':'Call deferred',change,score};});
}
export function chartsPanel(world:World) {
  const karmenia=karmeniaRelations(world); const latest=karmenia.at(-1); const rows=karmenia.length?karmenia.map(row=>`<tr><th>Day ${row.day}</th><td>${row.position}</td><td>${row.change>0?'+':''}${row.change}</td><td>${row.score}</td></tr>`).join(''):'<tr><td colspan="4">No Karmenia position recorded yet. The security channel will appear as the storyline develops.</td></tr>';
  return `${revenueMarkup(world)}${electionMarkup(world)}<p class="chart-explainer">Every point is a recorded day. Hover or focus a point for its value. Approval and relations use fixed scales; other charts scale to their history.</p><div class="trend-grid">${metrics.map(metric=>chartMarkup(world,metric.key)).join('')}</div><section class="relations-table"><div class="trend-heading"><span>Karmenia relations</span><span class="trend-change">${latest?`Current standing: ${latest.score}`:'No position yet'}</span></div><p>Tracks how Artem Volkov interprets Freedoma’s public security choices. This is separate from Petrovia’s oil relationship.</p><div class="table-scroll"><table><caption>Karmenia diplomatic positions</caption><thead><tr><th>Day</th><th>Freedoma position</th><th>Change</th><th>Standing</th></tr></thead><tbody>${rows}</tbody></table></div></section><details class="chart-data"><summary>View chart data as a table</summary><div class="table-scroll"><table><caption>Daily state, including the starting position</caption><thead><tr><th>Day</th>${metrics.map(m=>`<th>${m.label} (${m.unit})</th>`).join('')}</tr></thead><tbody>${[createWorld().state,...world.events.filter(e=>!e.cabinetAssignment&&(!e.strategic||e.strategic.command.kind==='overnight'||e.strategic.reports.length)).map(e=>e.after)].map(state=>`<tr><th>${state.day}</th>${metrics.map(m=>`<td>${state[m.key]}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details>`;
}
