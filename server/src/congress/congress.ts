...
export type CongressAction='draft'|'costing'|'benefits'|'oversight'|'vote';
...
export class CongressRules {
 ...
 options(world:World,index:number,chamber:Chamber){const bill=this.bill(world,index);this.tally(world,index,chamber);const acted=world.events.some(e=>e.after.day===world.state.day&&e.congress?.kind==='action');const options=bill.proposal?[
 ...
 ].filter(o=>!bill.amendments.includes(o.id)&&!(o.id==='vote'&&bill.passed.includes(chamber))):[{id:'draft' as const,title:'Submit a practical proposal',hint:'Describe a feasible plan connected to this promise. Submission does not fulfill it.',cost:0}];
 return options.map(o=>({...o,blocked:world.state.day>30?'Your term has ended.':bill.authorized?'Both chambers approved this promise.':acted?'Today’s congressional action is complete. Continue tomorrow.':world.state.treasury<o.cost?'Not enough treasury.':null}));
 }
