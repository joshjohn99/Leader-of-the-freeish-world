...
export class WorldEventEngine {
 ...
 next(world:World):NewsBulletin|undefined {
  ...
  const seed=JSON.stringify(world.events.find(e=>e.campaign?.kind==='launch')?.campaign??'freedoma')+':'+day+':'+world.events.length;
 ...
 context(world:World){return {bulletins:world.events.filter(e=>e.news).slice(-5).map(e=>e.news),responses:world.events.filter(e=>e.newsResponse).slice(-8).map(e=>({day:e.after.day,...e.newsResponse})),rule:'PNN reports recorded events only. Border disputes are not invasions. Risks are not completed outcomes. No automatic import loss or currency collapse. Responses are bounded actions, not guaranteed resolutions.'};}
}
