...
export const agendaEngine=new AgendaEngine()
 .register({id:'public',task:world=>({id:'public',title:'Attend the press briefing',reason:world.state.approval<40?'Voters need an answer. Address a promise, announce a decision, or release a statement.':'Set today’s public direction: address a promise, choose a policy, or release an announcement.',panel:'press'}),completed:(_task,events)=>events.some(e=>e.congress?.kind==='action'||e.announcement||e.campaign?.kind==='decision'||(!e.diplomacy&&!e.sterling&&!e.newsResponse&&!e.agenda&&!e.congress&&e.timing==='day-action'))})
