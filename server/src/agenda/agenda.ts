import {strategicState,strategicAgendaReason} from '../diplomacy/strategic.ts';
...
export const agendaEngine=new AgendaEngine()
 .register({id:'public',task:world=>({id:'public',title:'Attend the press briefing',reason:world.state.approval<40?'Voters need an answer. Address a promise, announce a decision, or release a statement.':'Set today’s public direction: address a promise, choose a policy, or release an announcement.',panel:'press'}),completed:(_task,events)=>events.some(e=>e.congress?.kind==='action'||e.announcement||e.campaign?.kind==='decision'||(!e.strategic&&!e.diplomacy&&!e.sterling&&!e.newsResponse&&!e.agenda&&!e.congress&&e.timing==='day-action'))})
 .register({id:'petrov',task:world=>strategicState(world)?(strategicAgendaReason(world)?{id:'petrov',title:'Work with Petrov',reason:strategicAgendaReason(world)!,panel:'phone'}:undefined):world.state.oil<12||world.state.relations<0||world.state.day%3===0?{id:'petrov',title:'Work with Petrov',reason:world.state.oil<12?'Fuel reserves need attention. Negotiate a shipment or explicitly decide how to leave the talks.':'Diplomatic tensions need a response. Settle on a position or decline the deal.',panel:'phone'}:undefined,completed:(_task,events)=>events.some(e=>(!!e.strategic&&['sign','end_call','spot','suspend'].includes(e.strategic.command.kind))||(e.diplomacy&&!['counter','ask_needs','challenge'].includes(e.diplomacy.reply)))})
